...
Scripts install Node.js 18+, mpv, and yt-dlp if missing. Alternatives: `npm install -g fahy-cli`, or clone + `npm link`. Update with `fahy upgrade`, remove with `fahy uninstall` (`--purge` also deletes local data).
